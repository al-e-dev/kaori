...
export default new class Download {
    constructor() {
        ...
        this._extract = data =>
            data
                ? JSON.parse(data.split("var ytInitialData = ")[1].split("</")[0].slice(0, -1))
                : new Error("No data provided")
        this._date = date =>
            new Date(Date.parse(date.split(" ").slice(1).join(" "))).toISOString()
        this._convert = value =>
    ...
    search(query) {
        return new Promise(async resolve => {
            await this.client
                ...
                .then(({ data }) => {
                    const contents =
                        this._extract(data).contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents[0]
                            ...
                            if (tag === "videoRenderer") {
                                ...
                                return {
                                    ...
                                    description: data.descriptionSnippet
                                        ? data.descriptionSnippet.runs.map(run => run.text).join("")
                                        : data.detailedMetadataSnippets
                                            ? data.detailedMetadataSnippets[0].snippetText.runs.map(run => run.text).join("")
                                            : "",
                                    viewers: data.viewCountText?.simpleText,
                                    verified:
                                        data.ownerBadges &&
                                        data.ownerBadges.some(
                                            badge => badge.metadataBadgeRenderer.tooltip === "Official Artist Channel"
                                        ),
                                    duration: data.lengthText?.accessibility?.accessibilityData?.label,
                                    thumbnail: data.thumbnail?.thumbnails[0]?.url,
                                    moving_thumbnail: data.richThumbnail
                                        ? data.richThumbnail.movingThumbnailRenderer.movingThumbnailDetails.thumbnails[0].url
                                        : null,
                                    avatar:
                                        data.channelThumbnailSupportedRenderers?.channelThumbnailWithLinkRenderer?.thumbnail
                                            ?.thumbnails[0]?.url,
            ...
            await this.client
                ...
                .then(async ({ data }) => {
                    ...
                    const result = {
                        ...
                        description:
                            c.find(item => item.videoSecondaryInfoRenderer)?.videoSecondaryInfoRenderer?.attributedDescription
                                ?.content || "No description",
                        date: this._date(v.dateText.simpleText),
                        views: this._convert(v.viewCount.videoViewCountRenderer.viewCount.simpleText),
                        likes: this._convert(
                            v.videoActions?.menuRenderer?.topLevelButtons?.find(
                                btn =>
                                    btn.segmentedLikeDislikeButtonViewModel?.likeButtonViewModel?.likeButtonViewModel
                            )?.segmentedLikeDislikeButtonViewModel.likeButtonViewModel.likeButtonViewModel
                                .toggleButtonViewModel.toggleButtonViewModel.toggledButtonViewModel.buttonViewModel.title
                        ),
    ...
    convert(url, quality) {
        return new Promise(async resolve => {
            const id = this.getYouTubeID(url)
            await this.client
                .get(`https://ytdl.vreden.web.id/convert.php/${id}/${quality}`)
                .then(async ({ data: x }) => {
                    await this.client
                        .get(`https://ytdl.vreden.web.id/progress.php/${x.convert}`)
                        .then(async ({ data: y }) => {
                            if (y.status === "Error")
                                throw new Error("Conversion progress encountered an error")
    ...
    ytmp3(url) {
        return new Promise(async resolve => {
            ...
            await this.convert(url, 320)
                .then(async ({ url, filename, quality }) => {
