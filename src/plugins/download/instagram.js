...
export default {
    ...
    exec: async (m, {sock}) => {
        ...
        if (dec(result) === 'object' && media(result.url[0].type)) {
            await sock.sendMessage(m.from, { [media(result.url[0].type)]: { url: result.url[0].url }, caption: `*· Me gusta* – ${result.meta.like_count}
*· Comentarios* – ${result.meta.comment_count}
*· Creador* – ${result.meta.username}` })
        } else if (dec(result) === 'array' && media(result.url[0].type)) {
