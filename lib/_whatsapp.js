	...
	const sock = Object.defineProperties(makeWASocket(args), {
		...
		sendMedia: {
			async value(from, path, opts = {}) {
				...
				let type = opts.document
					? "document" : opts.sticker || mime.split("/")[1] === "webp"
						? "sticker" : /image/.test(mime)
							? "image" : /video/.test(mime)
								? "video" : /audio/.test(mime)
									? "audio" : "document";

				if (type === 'audio' && opts.convert) {
					const raw = tmp.tmpNameSync({ postfix: '.raw' });
					const out = tmp.tmpNameSync({ postfix: '.ogg' });
					fs.writeFileSync(raw, buffer);
					try {
						await sock.convertToOgg({ raw, out });
						buffer = fs.readFileSync(out);
						mime = 'audio/ogg';
					} catch (e) {
						console.error(e);
					} finally {
						[raw, out].forEach(f => fs.existsSync(f) && fs.unlinkSync(f));
					}
				}
				...
				return await sock.sendMessage(from, {
					...
					ptt: opts.ptt && type === "audio" ? opts.ptt : null,
					gifPlayback: type === "video" && opts.gif ? opts.gif : null,
					mimetype: opts.mime || mime,
				}, { quoted: opts.quoted || null });
			},
			enumerable: true,
		},
		convertToOgg: {
			value(paths, isPcm = true) {
				return new Promise((resolve, reject) => {
					ffmpeg(paths.raw, { timeout: 60 })
