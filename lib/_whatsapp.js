		...
		sendMedia: {
			async value(from, path, options = {}) {
				...
				if (Buffer.isBuffer(path)) { 
					buffer = path 
				} else if (typeof path === 'string' && path.startsWith('https')) {
				...
				let size = Function.byteToSize(Size)
				...
				// Convertir a archivo de audio compatible si es necesario
				if (type === 'audio' && options.convert) {
					const tempFilePath = path.replace(ext, 'ogg');
